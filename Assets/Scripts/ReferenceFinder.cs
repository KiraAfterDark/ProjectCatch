...
public class ReferenceFinder : EditorWindow
{
	...
	private List<SearchReference> references = new List<SearchReference>();
	...
	static void FindObjectReferences()
	{
		...
		window.FindObjectReferences(Selection.objects);
	}
	...
	void OnGUI () 		
	{
		foreach (var reference in references)
		{
			...
			GUILayout.Label(reference.source.name);
			GUILayout.Label("Found: " + reference.references.Count);
			...
			for (int i = reference.references.Count - 1; i >= 0; --i)
			{
				LayoutItem(i, reference.references[i]);
			}
			...
			if (m_findReferencesAfterLayout != null)
			{
				...
				var r = new SearchReference
				{
					source = m_findReferencesAfterLayout,
					references = new List<GameObject>()
				};
				references.Add(r);
				
				FindObjectReferences(m_findReferencesAfterLayout, r.references);
	...
	private void FindObjectReferences(Object[] toFind)
	{
		...
		foreach (Object obj in toFind)
		{
			...
			references.Add(r);
			FindObjectReferences(obj, r.references);
		}
	}

	/// Finds references to passed objects and puts them in m_references
	void FindObjectReferences( Object toFind , List<GameObject> referenceList)
	{			
		...
		for ( int i = 0; i < numPaths; ++i )
		{
			...
			if ( tmpArray != null && tmpArray.Length > 0 && tmpArray[0] != toFind) // Don't add self
			{
				...
				{						
					...
					referenceList.Add(tmpArray[0] as GameObject);
				}
		...
		for ( int i = referenceList.Count - 1; i >= 0; i-- ) 
		{
			...
			for ( int j = 0; j < dependencies.Length && shouldRemove == false; ++j )
			{
				...
				shouldRemove =  ( referenceList.Find( item => item == dependency && item != tmpArray[0] ) != null );
			}
